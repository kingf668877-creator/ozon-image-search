// 查看 entrypoint API 返回的第一个商品的完整字段，找到图片字段
...
(async () => {
  ...
  const expr = `
    (async () => {
      ...
      if (!tileKey) return JSON.stringify({ error: 'no tileGrid' });
      ...
      for (const k of Object.keys(first)) {
        const v = first[k];
        const t = typeof v;
        if (t === 'string') fields[k] = v.slice(0, 200);
        else if (t === 'number' || t === 'boolean') fields[k] = v;
        else if (Array.isArray(v)) fields[k] = '[' + v.length + ' items]';
        else if (v && t === 'object') fields[k] = '{' + Object.keys(v).join(',') + '}';
        else fields[k] = String(v);
      }
      return JSON.stringify({ fields, firstImage: first.image, firstImages: first.images, firstMedia: first.media, imageKeys: Object.keys(first).filter(k => k.toLowerCase().includes('image') || k.toLowerCase().includes('img') || k.toLowerCase().includes('media') || k.toLowerCase().includes('pic')) }, null, 2);
    })()
  `;
